...
const Reminders = () => {
  ...
  // ✅ SAFE DATE FORMATTER (no crashing)
  ...
  // ✅ REAL-TIME POLLING (every 10 sec)
  useEffect(() => {
    if (!user?.id) return;
    ...
    const loadRemindersAndSync = async () => {
      try {
        ...
        // 🔔 Trigger toast ONLY for new reminders (after first load)
        if (
          reminderList.length > prevCountRef.current &&
